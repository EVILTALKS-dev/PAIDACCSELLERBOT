...
from keyboards import (
    ...
    admin_account_kb, admin_approve_kb, user_ban_kb
)
...
class AddAccState(StatesGroup):
    number      = State()
    country     = State()   # Free text — admin types any country name + flag
    price       = State()
    password    = State()
    twofa       = State()
    session_str = State()
...
async def add_number(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "✅ Number saved!\n\n"
        "<b>Step 2/7 — Country aur Flag likho:</b>\n\n"
        "Format: <code>Flag CountryName</code>\n\n"
        "Examples:\n"
        "<code>🇮🇳 India</code>\n"
        "<code>🇺🇸 USA</code>\n"
        "<code>🇷🇺 Russia</code>\n"
        ...
        "<code>🇵🇰 Pakistan</code>\n"
        "<code>🇬🇧 UK</code>\n\n"
        "Koi bhi country likh sakte ho — koi limit nahi!",
        parse_mode="HTML"
    )
...
async def add_country(msg: Message, state: FSMContext):
    ...
    # Split flag and country name
    # User types: "🇮🇳 India" → flag=🇮🇳, country=India
    parts = text.split(None, 1)  # split on first whitespace
    if len(parts) == 2:
        flag    = parts[0].strip()
        country = parts[1].strip()
    else:
        # No flag given — use globe
        flag    = "🌍"
        country = text
...
async def add_price(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        f"✅ Price: ₹{price:.2f}\n\n"
        f"<b>Step 4/7 — Account Password</b>\n"
        f"(ya <code>skip</code> type karo):",
        parse_mode="HTML"
    )
...
async def add_password(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "✅ Password saved!\n\n"
        "<b>Step 5/7 — 2FA Password</b>\n"
        "(ya <code>skip</code> type karo):",
        parse_mode="HTML"
    )
...
async def add_twofa(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "✅ 2FA saved!\n\n"
        "<b>Step 6/7 — Telethon Session String</b>\n"
        "(Auto OTP ke liye)\n\n"
        "Session string generate karne ke liye:\n"
        "<code>python session_gen.py</code> locally chalao\n\n"
        "Ya <code>skip</code> karo (manual OTP fallback hoga):",
        parse_mode="HTML"
    )
...
async def add_session(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "<b>Step 7/7 — Description</b>\n"
        "(ya <code>skip</code> type karo)\n\n"
        "Example: <i>Fresh account, 2024 created, verified</i>",
        parse_mode="HTML"
    )
...
async def add_done(msg: Message, state: FSMContext):
    ...
    session_status = "✅ Session Added (Auto OTP)" if d.get("session_str") else "⚠️ No Session (Manual OTP)"
    ...
    await msg.answer(
        ...
        f"🔑 Password: {d.get('password') or 'N/A'}\n"
...
async def view_accounts(msg: Message):
    ...
    for acc in accounts[:20]:
        ...
        sess  = "📡 ✅ Auto OTP" if acc.get("session_str") else "📡 ❌ No Session"
        text  = (
...
async def edit_price_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.answer(
        f"✏️ Account #{acc_id} ka naya price enter karo:",
        reply_markup=cancel_kb()
    )
    await cq.answer()
...
async def edit_session_done(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "✅ Session updated!" if sess else "✅ Session removed!",
        reply_markup=admin_main_kb()
    )
...
async def approve(cq: CallbackQuery, bot: Bot):
    ...
    await cq.message.edit_caption(
        ...
        parse_mode="HTML"
    ) if cq.message.caption else await cq.message.edit_text(
        f"✅ <b>Order #{order_id} Approved!</b>\n"
        f"📱 {acc['number']} delivered.",
        parse_mode="HTML"
    )
...
async def pending(msg: Message):
    ...
    for o in orders:
        ...
        await msg.answer(
            ...
            f"💸 ₹{o['amount']:.2f}\n"
            f"🗓 {o['created_at'][:19]}\n"
            f"📸 Screenshot: {'✅ Uploaded' if o.get('screenshot') else '❌ Not uploaded'}",
            parse_mode="HTML",
...
async def stats(msg: Message):
    ...
    country_text = ""
    for cs in stock:
        country_text += f"  {cs['flag']} {cs['country']}: {cs['count']} acc · ₹{cs['price']:.0f}\n"
    ...
    await msg.answer(
        ...
        f"{country_text if country_text else '  No stock available'}",
        parse_mode="HTML"
    )
...
async def otp_sessions(msg: Message):
    ...
    for s in sessions:
        ...
        await msg.answer(
            ...
            f"📡 Session: {'✅ Auto OTP' if acc and acc.get('session_str') else '❌ Missing — Manual needed'}\n"
            f"🗓 {s['created_at'][:19]}",
            parse_mode="HTML",
